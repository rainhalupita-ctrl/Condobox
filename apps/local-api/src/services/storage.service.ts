...
export class StorageService {
   ...
   */
  async saveLabelImage(buffer: Buffer, originalExt: string = 'jpg'): Promise<{ relativePath: string; fullPath: string; url: string; filename: string }> {
   ...
   */
  async saveSignatureImage(data: string | Buffer): Promise<{ relativePath: string; fullPath: string; url: string; filename: string }> {
   ...
   */
  async cleanupOldLabels(retentionDays: number = 90): Promise<{ deletedCount: number }> {
    let deletedCount = 0;
    const now = Date.now();
    const maxAgeMs = retentionDays * 24 * 60 * 60 * 1000;

    const self = this;
    async function walkAndClean(dir: string, bucket: string, baseDir: string) {
