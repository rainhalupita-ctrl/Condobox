...
export async function packageRoutes(fastify: FastifyInstance) {
  ...
  fastify.post('/api/packages/:id/notify', async (request, reply) => {
    ...
    try {
      ...
      let pkg: any = databaseService.getPackageById(id) || databaseService.getPackageByQrTokenOrCode(id);
      ...
      if (!phone && pkg.unit_id) {
        ...
        if (primary?.phone) {
          phone = primary.phone;
          if (!pkg.recipient_name_ocr) residentName = primary.name;
        }
      }
      ...
      if (notifyRes.success) {
        try { databaseService.updatePackageStatus(pkg.id, 'NOTIFIED'); } catch {}
        return reply.send({
          success: true,
  ...
  fastify.patch('/api/packages/:id/status', async (request, reply) => {
    ...
    try {
      ...
      if (supabaseService.isConfigured()) {
        try {
          const client = supabaseService.getClient();
          const updateData: any = { status: status || 'RETURNED' };
          if (reason) {
